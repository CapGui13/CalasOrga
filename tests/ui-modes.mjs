...
try{
  ...
  assert.equal(await desktop.evalJs(`(()=>{const p=document.querySelector('#adminScheduleBody tr.day-past');if(!p)return [...document.querySelectorAll('#adminScheduleBody tr[data-date]')].every(r=>r.dataset.date>=(()=>{const q=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Paris',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),g=t=>q.find(x=>x.type===t)?.value;return g('year')+'-'+g('month')+'-'+g('day')})());const f=[...document.querySelectorAll('#adminScheduleBody tr')].find(r=>!r.classList.contains('day-past')&&!r.classList.contains('week-separator'));return !!f&&getComputedStyle(p.querySelector('.role-cell')).backgroundColor!==getComputedStyle(f.querySelector('.role-cell')).backgroundColor})()`),true,'desktop admin must skip a fully elapsed current week, while any displayed past rows stay visibly greyed');
  ...
  assert.equal(await desktopMember.evalJs(`(()=>{const p=document.querySelector('#memberRoot .member-schedule tr.day-past');if(!p)return [...document.querySelectorAll('#memberRoot .member-schedule tbody tr[id^="date-"]')].every(r=>r.id.slice(5)>=(()=>{const q=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Paris',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),g=t=>q.find(x=>x.type===t)?.value;return g('year')+'-'+g('month')+'-'+g('day')})());const f=[...document.querySelectorAll('#memberRoot .member-schedule tbody tr')].find(r=>!r.classList.contains('day-past')&&!r.classList.contains('week-separator'));if(!f)return false;const b=p.querySelector('.member-role-button');return getComputedStyle(b).cursor==='not-allowed'&&getComputedStyle(p.querySelector('.role-cell')).backgroundColor!==getComputedStyle(f.querySelector('.role-cell')).backgroundColor})()`),true,'desktop member must skip a fully elapsed current week, while any displayed past dates stay read-only and greyed');
  ...
  assert.equal(await tabletNarrow.evalJs(`(()=>{const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Paris',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),g=t=>parts.find(p=>p.type===t)?.value,today=g('year')+'-'+g('month')+'-'+g('day'),shown=[...document.querySelectorAll('#adminScheduleMobile .mobile-date-card')].map(c=>c.dataset.date),[y,m,d]=today.split('-').map(Number),dt=new Date(Date.UTC(y,m-1,d)),wd=dt.getUTCDay(),daysToMonday=wd===0?1:8-wd,next=new Date(dt);next.setUTCDate(dt.getUTCDate()+daysToMonday);const nextMonday=next.getUTCFullYear()+'-'+String(next.getUTCMonth()+1).padStart(2,'0')+'-'+String(next.getUTCDate()).padStart(2,'0'),remaining=[];for(let i=0;i<=6-wd;i++){const x=new Date(dt);x.setUTCDate(dt.getUTCDate()+i);if([1,2,4].includes(x.getUTCDay()))remaining.push(x)}return remaining.length?true:shown.every(s=>s>=nextMonday)})()`),true,'tablet admin must start on next week once the current week has no open day left');
  ...
  assert.equal(await tabletNarrow.evalJs(`(()=>{const p=document.querySelector('#adminScheduleMobile .mobile-date-card.day-past > .admin-calendar-edit');if(!p)return true;const f=document.querySelector('#adminScheduleMobile .mobile-date-card:not(.day-past) > .admin-calendar-edit');return !!f&&getComputedStyle(p).backgroundColor!==getComputedStyle(f).backgroundColor&&getComputedStyle(p).cursor==='not-allowed'})()`),true,'past tablet full-day edit buttons must be greyed like the rest of the card when past dates are displayed');
  ...
  assert.equal(await tabletNarrow.evalJs(`(()=>{const p=document.querySelector('#adminScheduleMobile .mobile-date-card.day-past');if(!p)return true;const f=[...document.querySelectorAll('#adminScheduleMobile .mobile-date-card')].find(x=>!x.classList.contains('day-past'));return !!f&&getComputedStyle(p).backgroundColor!==getComputedStyle(f).backgroundColor})()`),true,'past tablet dates must be visibly greyed when they are displayed');
  ...
  assert.equal(await tabletMember.evalJs(`(()=>{const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Paris',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),g=t=>parts.find(p=>p.type===t)?.value,today=g('year')+'-'+g('month')+'-'+g('day'),cards=[...document.querySelectorAll('#scheduleMobile .mobile-date-card')],shown=cards.map(c=>c.id.replace('mobile-date-','')),[y,m,d]=today.split('-').map(Number),dt=new Date(Date.UTC(y,m-1,d)),wd=dt.getUTCDay(),daysToMonday=wd===0?1:8-wd,next=new Date(dt);next.setUTCDate(dt.getUTCDate()+daysToMonday);const nextMonday=next.getUTCFullYear()+'-'+String(next.getUTCMonth()+1).padStart(2,'0')+'-'+String(next.getUTCDate()).padStart(2,'0'),remaining=[];for(let i=0;i<=6-wd;i++){const x=new Date(dt);x.setUTCDate(dt.getUTCDate()+i);if([1,2,4].includes(x.getUTCDay()))remaining.push(x)}return remaining.length?true:shown.every(s=>s>=nextMonday)})()`),true,'tablet member view must start on next week once the current week has no open day left');
