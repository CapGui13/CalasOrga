...
const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'calasorga-v1544-'));
...
try{
  ...
  assert.equal(health.appVersion,'0.15.44.0-multidevice-hardening-vercel');
  ...
  // Hardening V15.44 : pas de PII réelle dans les sources distribués.
  assert.doesNotMatch(indexText,/(?:gmail\.com|yahoo\.fr|wanadoo\.fr|laposte\.net|orange\.fr|free\.fr)/i);
  ...
  assert.match(indexText,/function usesSingleActivation\(event\)/);
  assert.match(indexText,/event\?\.detail===0/);
  assert.match(indexText,/pointerType==='touch'\|\|pointerType==='pen'/);
  // Désactivation sécurisée par la voie confirmée existante.
  assert.match(indexText,/toggleMemberActiveDirect[\s\S]{0,180}return setMemberActive\(id,active,name,btn\)/);
  assert.match(indexText,/Désactiver un membre/);
  // Cibles tactiles et aide explicite.
  assert.match(indexText,/day-assignment-remove[\s\S]{0,180}width:44px!important/);
  assert.match(indexText,/member-state-pill\{min-height:44px!important/);
  assert.match(indexText,/Touchez un membre, puis le poste à lui attribuer/);
  assert.match(indexText,/safe-area-inset-left/);
  assert.match(indexText,/safe-area-inset-right/);
